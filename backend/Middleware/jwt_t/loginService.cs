...
namespace backend.Middleware.jwt_t
{
    ...
    public class loginService : IUserService_T
    {
        ...
        {
            sha256Hash sha256=new sha256Hash();
            var travelUser=_user.SingleOrDefault(m=>m.Account==model.Account && m.Password==sha256.getSha256(model.Password,this._appSettings.hash_key));
        ...
        public Members GetDataById(string Account)
        {
            return _user.FirstOrDefault(m => m.Account == Account);
        }

        public string generateJwtToken(Members user,List<IDictionary<string,string>> role)
        {
            ...
            {
                Subject=new ClaimsIdentity(new[] {new Claim("Account",user.Account.ToString()),new Claim("role", GetLine(role)) }),
